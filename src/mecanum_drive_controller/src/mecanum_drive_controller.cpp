  ...
  const rclcpp::Duration & period)
{
  ...
  if (params_.open_loop) {
    ...
    const double front_left_feedback =
      registered_front_left_wheel_handle_->feedback.get().get_value();
    const double front_right_feedback =
      registered_front_right_wheel_handle_->feedback.get().get_value();
    const double rear_left_feedback =
      registered_rear_left_wheel_handle_->feedback.get().get_value();
    const double rear_right_feedback =
      registered_rear_right_wheel_handle_->feedback.get().get_value();

    if (std::isnan(front_left_feedback) || std::isnan(front_right_feedback) ||
      std::isnan(rear_left_feedback) ||
    ...
    if (params_.position_feedback) {
      odometry_.update(
        front_left_feedback, front_right_feedback, rear_left_feedback,
        rear_right_feedback, time);
    } else {
  ...
  limiter_linear_x_.limit(
    linear_command_x, last_command.linear.x, second_to_last_command.linear.x,
    period.seconds());
  limiter_linear_y_.limit(
    linear_command_y, last_command.linear.y, second_to_last_command.linear.y,
    period.seconds());
  ...
  const double velocity_front_left =
    (linear_command_x - linear_command_y - (wheel_separation_x + wheel_separation_y) / 2. *
    angular_command) /
    wheel_radius;
  ...
  const rclcpp_lifecycle::State &)
{
  ...
  if (front_left_wheel_name_.empty() || front_right_wheel_name_.empty() ||
    rear_left_wheel_name_.empty() ||
  ...
  std::copy(
    twist_diagonal.begin(), twist_diagonal.end(),
    params_.twist_covariance_diagonal.begin());
  ...
  try {
    ...
    RCLCPP_ERROR(
      get_node()->get_logger(), "Error configuring x linear speed limiter: %s",
      e.what());
  }
  ...
  try {
    ...
    RCLCPP_ERROR(
      get_node()->get_logger(), "Error configuring y linear speed limiter: %s",
      e.what());
  }
...
MecanumDriveController::configure_wheel(
