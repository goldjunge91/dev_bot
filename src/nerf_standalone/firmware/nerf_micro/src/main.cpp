...
ESCCalibration calibrator(nerf.getLeftESC(), nerf.getRightESC());
#endif

// --- CALIBRATION HELPERS (called by Comms) ---
void runCalibrateMax() {
#ifdef DEBUG_CALIBRATION
    calibrator.calibrateMax();
#else
    SerialOutput::print(F("ERR: Calibration disabled (Define DEBUG_CALIBRATION to enable)"));
#endif
}

void runCalibrateMin() {
#ifdef DEBUG_CALIBRATION
    calibrator.calibrateMin();
#else
    SerialOutput::print(F("ERR: Calibration disabled"));
#endif
}
