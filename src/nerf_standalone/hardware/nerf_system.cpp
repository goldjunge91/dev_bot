...
bool NerfComms::connected() const { return serial_conn_.IsOpen(); }  // Status der Verbindung
...
NerfSystem::export_state_interfaces() {
  ...
  for (const auto &joint : info_.joints) {
    for (const auto &interface : joint.state_interfaces) {
      double *state_ptr = get_state_ptr(joint.name, interface.name);  // Hole Zeiger auf State-Variable
      if (!state_ptr) {
...
NerfSystem::export_command_interfaces() {
  ...
  for (const auto &joint : info_.joints) {
    for (const auto &interface : joint.command_interfaces) {
      double *command_ptr = get_command_ptr(joint.name, interface.name);  // Hole Zeiger auf Command-Variable
      if (!command_ptr) {
        RCLCPP_ERROR(
...
NerfSystem::read(const rclcpp::Time & /*time*/,
  ...
  auto sanitize = [](double &v) {
    if (!std::isfinite(v))
      v = 0.0;  // Setze auf 0 bei ungültigem Wert
  };
...
NerfSystem::write(const rclcpp::Time & /*time*/,
  ...
  if (std::abs(delta) > 0.01) {
    ...
    // Update Feedback sofort (Open Loop)
  ...
  double avg_vel = (std::abs(hw_commands_.flywheel_l_vel) +
                    std::abs(hw_commands_.flywheel_r_vel)) / 2.0;
