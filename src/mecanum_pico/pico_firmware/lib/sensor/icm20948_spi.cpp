...
bool Icm20948Simple::initialize() {
    ...
    printf("[ICM20948] SPI: CS=GP%d SCK=GP%d MOSI=GP%d MISO=GP%d @ %lu Hz\n",
           config_.cs_pin,
           config_.sck_pin,
           config_.mosi_pin,
           config_.miso_pin,
    ...
    for (int attempt = 1; attempt <= 3 && !success; attempt++) {
        ...
        printf("[ICM20948] WHO_AM_I attempt %d: 0x%02X (expected 0x%02X)\n",
               attempt,
               whoami,
               WHO_AM_I_RESPONSE);
...
bool Icm20948Simple::readAcceleration(Vec3 &accel_g) {
    ...
    uint8_t buf[6];
    if (!readRegisters(REG_ACCEL_XOUT_H, buf, 6)) {
        return false;
    }
...
bool Icm20948Simple::readTemperature(float &temperature_c) {
    ...
    uint8_t buf[2];
    if (!readRegisters(REG_TEMP_OUT_H, buf, 2)) {
        return false;
    }
