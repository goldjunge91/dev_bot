...
public:
     ...
     * @return true on successful parse, false on timeout or malformed response.
     */
    bool read_imu_values(double &ax, double &ay, double &az, double &gx, double &gy, double &gz) {
     ...
     */
    virtual bool parse_imu_response(const std::string &line,
                                    double &ax,
                                    double &ay,
                                    double &az,
                                    double &gx,
                                    ...
                                    double &gz) {
        int parsed =
            std::sscanf(line.c_str(), "%lf %lf %lf %lf %lf %lf", &ax, &ay, &az, &gx, &gy, &gz);
        return parsed == 6;
    }
