...
public:
    ...
    void update(double ax, double ay, double az, double gx, double gy, double gz, double dt) {
        if (!std::isfinite(ax) || !std::isfinite(ay) || !std::isfinite(az) || !std::isfinite(gx) ||
            !std::isfinite(gy) || !std::isfinite(gz) || !std::isfinite(dt)) {
            return;
        }

        const double accel_norm = std::sqrt(ax * ax + ay * ay + az * az);
        const bool accel_trusted = accel_norm > kAccelTrustLow && accel_norm < kAccelTrustHigh;
