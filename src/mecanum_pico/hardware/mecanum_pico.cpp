...
hardware_interface::CallbackReturn MecanumPicoHardware::on_init(
    ...
    for (const hardware_interface::ComponentInfo &joint : info_.joints) {
        if (joint.command_interfaces.size() != 1) {
            RCLCPP_FATAL(rclcpp::get_logger("MecanumPicoHardware"),
    ...
    if (!info_.sensors.empty()) {
        ...
        static const char * const kExpectedImuInterfaces[10] = {"linear_acceleration.x",
        ...
        for (size_t i = 0; i < n_ifaces; ++i) {
            if (sensor.state_interfaces[i].name != kExpectedImuInterfaces[i]) {
                RCLCPP_FATAL(rclcpp::get_logger("MecanumPicoHardware"),
                             ...
                             sensor.name.c_str(),
                             i,
                             sensor.state_interfaces[i].name.c_str(),
        ...
        if (has_orientation_) {
            ...
            RCLCPP_WARN(rclcpp::get_logger("MecanumPicoHardware"),
...
// =============================================================================
std::vector<hardware_interface::StateInterface> MecanumPicoHardware::export_state_interfaces() {
    ...
    if (has_imu_sensor_) {
        static const char * const kImuInterfaceNames[6] = {"linear_acceleration.x",
                                                           "linear_acceleration.y",
                                                           "linear_acceleration.z",
                                                           "angular_velocity.x",
                                                           "angular_velocity.y",
...
// =============================================================================
std::vector<hardware_interface::CommandInterface> MecanumPicoHardware::export_command_interfaces() {
...
// =============================================================================
hardware_interface::return_type MecanumPicoHardware::read(const rclcpp::Time & /*time*/,
    ...
    if (has_imu_sensor_) {
        ...
        if (imu_ok) {
            ...
            if (has_orientation_) {
                imu_filter_.update(imu_data_[0],
                                   imu_data_[1],
                                   imu_data_[2],
                                   imu_data_[3],
                                   imu_data_[4],
                                   imu_data_[5],
                                   period.seconds());
                imu_filter_.quaternion(imu_orientation_[0],
                                       imu_orientation_[1],
                                       imu_orientation_[2],
                                       imu_orientation_[3]);
...
// =============================================================================
hardware_interface::return_type MecanumPicoHardware::write(const rclcpp::Time & /*time*/,
                                                           const rclcpp::Duration & /*period*/) {
    if (diagnostics_) {
        diagnostics_->set_connected(comms_.connected());
    }
    ...
    comms_.set_motor_values(to_ticks_per_loop(wheel_fl_),
...
PLUGINLIB_EXPORT_CLASS(mecanum_pico::MecanumPicoHardware, hardware_interface::SystemInterface)
