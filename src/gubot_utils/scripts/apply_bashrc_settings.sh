...
if [ "\$HOSTNAME" = "ros2pi" ]; then
    ...
    export CYCLONEDDS_URI="file://$WS_PATH/src/gubot_utils/cycloneDDS/raspi_cyclonedds.xml"
else
    echo "Identifiziert als PC (h3mistral/andere)"
    export CYCLONEDDS_URI="file://$WS_PATH/src/gubot_utils/cycloneDDS/pc_cyclonedds.xml"
fi
