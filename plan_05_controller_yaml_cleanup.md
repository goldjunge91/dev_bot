...
- [x] **1.1** `mecanum_drive_controller` Parameter-Schema ermitteln:
  ```bash
  # Im laufenden System:
  ros2 param list /mecanum_cont
  ros2 param describe /mecanum_cont wheels_radius
  ```
- [x] **1.2** Alternativ: ROS2 control Doku lesen:  
  `https://control.ros.org/rolling/doc/ros2_controllers/mecanum_drive_controller/doc/userdoc.html`
- [x] **1.3** Ergebnis notieren: welcher Wheel-Name-Parameter und welcher Kinematik-Pfad ist korrekt?

### Task 2 — Test schreiben (TDD: erst fehlschlagend)
- [x] **2.1** `src/gubot_one/test/test_controller_yaml.py` anlegen
- [x] **2.2** Test: YAML laden → prüfen dass für jedes Rad **genau ein** Wheel-Name-Parameter existiert  
  (kein `_command_joint_name` UND `_name` gleichzeitig)
- [x] **2.3** Test: `wheels_radius` erscheint **nicht** doppelt (einmal in `kinematics:`, einmal flach)
- [x] **2.4** Test: YAML ist valides Python-dict (schlägt bei Syntax-Fehlern fehl)
- [x] **2.5** Tests ausführen → **müssen rot sein** (Duplikate vorhanden)
  ```bash
  colcon test --packages-select gubot_one --pytest-args -k test_controller_yaml
  ```

### Task 3 — YAML bereinigen (KISS: minimale Änderung)
- [x] **3.1** Falsche Wheel-Name-Variante auskommentieren (die andere stehen lassen):
  ```yaml
...
- [x] **3.2** Kinematik-Duplikate: falsche Ebene auskommentieren:
  ```yaml
  # --- OLD (auskommentiert):
  ...
  ```
- [x] **3.3** Kommentar: `# Schema: mecanum_drive_controller v<x.y> — verified via ros2 param describe`

### Task 4 — Kinematik-Werte nach Plan 02 abstimmen
- [x] **4.1** `sum_of_robot_center_projection_on_X_Y_axis` = `wheel_offset_x + wheel_offset_y` neu berechnen  
  (alten Wert auskommentieren, neuen darunter)
- [x] **4.2** `wheels_radius` mit URDF-Geometrie abgleichen
...
- [x] **5.3** `ros2 param list /mecanum_cont` im laufenden System → keine Duplikate
