...
- [ ] **1.1** pytest-Testdatei anlegen: `src/gubot_one/test/test_urdf_geometry.py`
- [ ] **1.2** Test: XACRO rendern (`xacro gubot_one.urdf.xacro`), URDF parsen
- [ ] **1.3** Test: `front_left_wheel_joint` origin X == `+wheel_offset_x` (≠ 0)
- [ ] **1.4** Test: `front_right_wheel_joint` origin X == `+wheel_offset_x` (≠ 0)
- [ ] **1.5** Test: Symmetrie — front X == −rear X für beide Seiten
- [ ] **1.6** Tests ausführen → **müssen rot sein**
  ```bash
  colcon test --packages-select gubot_one --pytest-args -k test_urdf_geometry
  ```

### Task 2 — Analyse (KISS: minimale Änderung)
- [ ] **2.1** `gubot_one_geometry.xacro`: `wheel_offset_x` Property vorhanden oder neu anlegen?
- [ ] **2.2** Physische Maße prüfen: Abstand Roboter-Mitte → Vorderachse (Mechanik/CAD)
- [ ] **2.3** Husarion `rosbot_xl.urdf.xacro` lx/ly als Referenz

### Task 3 — Implementierung
- [ ] **3.1** Falls `wheel_offset_x` fehlt: Property mit korrektem Wert anlegen (z.B. `0.113`)
- [ ] **3.2** `front_left_wheel_joint` korrigieren (alte Zeile auskommentieren):
  ```xml
  <!-- OLD: <origin xyz="0 ${wheel_offset_y} 0" .../> -->
  <origin xyz="${wheel_offset_x} ${wheel_offset_y} 0" .../>
  ```
- [ ] **3.3** `front_right_wheel_joint` analog korrigieren
- [ ] **3.4** Hinterrad-Joints prüfen: müssen `−wheel_offset_x` haben (ggf. ebenfalls korrigieren + auskommentieren)

### Task 4 — Controller-YAML abstimmen
- [ ] **4.1** `mecanum_my_controllers.yaml`: `sum_of_robot_center_projection_on_X_Y_axis` neu berechnen  
  Formel: `sum = wheel_offset_x + wheel_offset_y`
- [ ] **4.2** Alten Wert auskommentieren, neuen Wert darunter einfügen

### Task 5 — Linter & Tests grün
- [ ] **5.1** XACRO Syntax: `ament_xmllint src/gubot_one/description/urdf/gubot_one_geometry.xacro`
